...
# --- AUTH (in-memory demo only, no real security!) ---
...
if st.session_state.user:
    ...
    # --- Add Asset ---
        ...
        with st.form("asset_form", clear_on_submit=True):
            ...
            if submit:
                asset = {
                    "id": len(st.session_state.assets),
    ...
    # --- Edit/Delete Asset ---
        ...
        if df.empty:
            ...
            if df_user.empty:
                st.info("No assets for your user.")
            else:
                ...
                with st.form("edit_form"):
                    ...
                    if update:
                        ...
                        # Update in session_state.assets
                        for idx, asset_obj in enumerate(st.session_state.assets):
                            if asset_obj["id"] == row_id:
                                st.session_state.assets[idx] = updated_asset
                                break
                        st.success("Asset updated!")

                    if delete:
                        st.session_state.assets = [
                            asset_obj for asset_obj in st.session_state.assets if asset_obj["id"] != row_id
                        ]
